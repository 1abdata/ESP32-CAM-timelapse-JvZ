...
//const char* ssid = "BigRedHouse2";
const char* ssid = "Elmwood-bg";
const char* password = "abcdefedcbabcdefedcbabcdef";

#define BelgiumTZ (+1)
#define WAstateTZ (-8)
#define YourTZ WAstateTZ
...
const long  gmtOffset_sec = 3600 * YourTZ;
...
//  ImageToSd();//1st pic of the timelapse (duplicate)
    tlrunningvalue  = true;
    resetTLcounter(); //JvZ 1st TL picture
  }
}
...
void ImageToSd()
{
  gpio_pulldown_dis(GPIO_NUM_4);    //JvZ re-config GPIO4
...
exit: writeLED(LEDStatus);  //JvZ restore calling state
}//end function
...
{
  LEDStatus = LEDon;
  pinMode(FOTO_FLASH, OUTPUT);    //config GPIO4 = LED driver
...
bool sdcardenable = true ; 
  ...
  writeLED(false);  //JvZ here after SD calls set SDMMC mode
...
void loop() {
  ...
  {
     //time is verstreken
    resetTLcounter(); //incl new picture
  }
}  //end loop
